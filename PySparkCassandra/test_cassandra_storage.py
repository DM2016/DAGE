...
def id_generator(size=6, chars=string.ascii_uppercase + string.digits + string.ascii_lowercase + string.punctuation):
...
for n in range(0, 1):
    for line in f:
        ...
        insert(id_generator(len(key)), id_generator(len(value)))
        # insert(key+"_2", value)
        count = count + 1
        if count % 100 == 0:
            print count
