...
def get_default_vpc(ec2_client):
    ...
    if len(vpcs) is not 1:
        raise VpcFailureException('No default VPC exists in this account and region!')
...
def get_security_group_by_name(ec2_client=None, name=None):
    ...
    return SecurityGroup(
        ec2_client=ec2_client, id=group[u'GroupId'], name=name,
...
def create_security_group(
    ...
    """creates a new security group with the specified vpc name group name
        ...
        and may delete an existing security group to avoid name conflict.
...
class SecurityGroup:
    ...
    def __init__(self, ec2_client=None, id=None, name=None,
...
def generate_cassandra_security_group(ec2_client):
    ...
    print "Found default VPC: " + d_vpc.get_id()

    security_group = create_security_group(
        ec2_client=ec2_client, name=security_group_template['GroupName'],
        description=security_group_template['Description'], vpc_id=d_vpc.get_id()
    )
    print "Created security group: " + security_group.get_id()

    ip_permissions = replace_group_id(
        ip_permissions=security_group_template['IpPermissions'],
